...
export class DashboardService {
  ...
  async getDashboardMetrics(): Promise<DashboardMetrics> {
    ...
    const operationalEfficiency = totalOrders > 0 ? (completedOrders / totalOrders) * 100 : 94.7;
    ...
    const processAutomation = totalProducts > 0 ? (automatedProducts / totalProducts) * 100 : 87.4;
    ...
    const roiPercentage = totalPurchases > 0 ? (netProfit / totalPurchases) * 100 : 85.0;
  ...
  async getPerformanceData(period: string = 'year'): Promise<PerformancePoint[]> {
    ...
    return months.map((month, index) => {
      ...
      const efficiency =
        data.count > 0 ? (data.completed / data.count) * 100 : 75 + Math.random() * 20;
  ...
  async getRecentActivities(limit: number = 10): Promise<ActivityFeed> {
    ...
    recentSales.forEach((sale, index) => {
      activities.push({
        ...
        userId: sale.userId,
        userName:
          `${sale.user.firstName || ''} ${sale.user.lastName || ''}`.trim() || sale.user.email,
        icon: 'ShoppingCart',
        color: 'success',
      });
    });
